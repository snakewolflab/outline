...
class GitHubWebhookTask extends _BaseTask.BaseTask {
  async perform(_ref) {
    let {
      headers,
      payload
    ...
    const plugin = plugins.find(p => p.value.service === _types.IntegrationService.GitHub);
    if (!plugin) {
      return;
    }
...
exports.default = GitHubWebhookTask;