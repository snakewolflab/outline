 ...
 */
const OAuthInterface = exports.OAuthInterface = {
  ...
  async generateAccessToken() {
    return `${_models.OAuthAuthentication.accessTokenPrefix}${_nodeCrypto.default.randomBytes(32).toString("hex")}`;
  },
  /**
  ...
  async generateRefreshToken() {
    return `${_models.OAuthAuthentication.refreshTokenPrefix}${_nodeCrypto.default.randomBytes(32).toString("hex")}`;
  },
  /**
  ...
  async generateAuthorizationCode() {
    return `${_models.OAuthAuthorizationCode.authorizationCodePrefix}${_nodeCrypto.default.randomBytes(32).toString("hex")}`;
  },
  /**
  ...
  async getAccessToken(accessToken) {
    const authentication = await _models.OAuthAuthentication.findByAccessToken(accessToken);
    if (!authentication) {
      return false;
    }
    return {
  ...
  async getRefreshToken(refreshToken) {
    let authentication = await _models.OAuthAuthentication.findByRefreshToken(refreshToken);
    if (!authentication) {
      ...
      authentication = await _models.OAuthAuthentication.findOne({
        where: {
      ...
      const reuseWindowStart = (0, _dateFns.subSeconds)(new Date(), refreshTokenReuseInterval);
      const withinReuseInterval = authentication?.deletedAt && authentication.deletedAt > reuseWindowStart;
      if (authentication?.grantId && !withinReuseInterval) {
        await Promise.all([_models.OAuthAuthentication.destroy({
          where: {
            grantId: authentication.grantId
          }
        }), _models.OAuthAuthorizationCode.destroy({
          where: {
            grantId: authentication.grantId
          }
        })]);
  ...
  async getAuthorizationCode(authorizationCode) {
    const code = await _models.OAuthAuthorizationCode.findByCode(authorizationCode);
    if (!code) {
      return false;
    }
    const oauthClient = await _models.OAuthClient.findByPk(code.oauthClientId);
    if (!oauthClient) {
      return false;
    }
  ...
  async getClient(clientId, clientSecret) {
    ...
    if (clientSecret && !(0, _crypto.safeEqual)(client.clientSecret, clientSecret)) {
      return false;
    }
    return {
      id: client.clientId,
  ...
  async saveToken(token, client, user) {
    ...
    const accessTokenHash = (0, _crypto.hash)(accessToken);
    const refreshTokenHash = refreshToken ? (0, _crypto.hash)(refreshToken) : undefined;
    await _models.OAuthAuthentication.create({
      accessTokenHash,
      refreshTokenHash,
      accessTokenExpiresAt,
      refreshTokenExpiresAt,
      ...
      userId: user.id,
      grantId: user.grantId || _nodeCrypto.default.randomUUID()
    });
    return {
      accessToken,
      accessTokenExpiresAt,
  ...
  async saveAuthorizationCode(code, client, user) {
    ...
    const authCode = await _models.OAuthAuthorizationCode.create({
      authorizationCodeHash: (0, _crypto.hash)(authorizationCode),
      expiresAt,
      scope,
      redirectUri,
      codeChallenge,
  ...
  async revokeToken(token) {
    const auth = await _models.OAuthAuthentication.findByRefreshToken(token.refreshToken);
    if (auth) {
      await auth.destroy();
      return true;
    }
  ...
  async revokeAuthorizationCode(code) {
    const authCode = await _models.OAuthAuthorizationCode.findByCode(code.authorizationCode);
    if (authCode) {
  ...
  async validateScope(user, client, scope) {
    ...
    return scopes.every(s => _AuthenticationHelper.default.isValidScope(s, {
      allowRootWildcard: false
    })) ? scopes : false;
  }
};