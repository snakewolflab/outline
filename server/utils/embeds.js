...
exports.checkEmbeddability = checkEmbeddability;
exports.convertBareUrlsToEmbedMarkdown = convertBareUrlsToEmbedMarkdown;
var _embeds = require("../../shared/editor/lib/embeds");
var _embeds2 = _interopRequireDefault(require("../../shared/editor/embeds"));
 ...
 */
function isBlockedByXFrameOptions(value) {
  if (!value) {
    return false;
  }
  ...
  return normalized === "DENY" || normalized === "SAMEORIGIN" || normalized.startsWith("ALLOW-FROM");
}
...
function isBlockedByCSP(value) {
  ...
  for (const directive of directives) {
    ...
    if (parts[0]?.toLowerCase() === "frame-ancestors") {
      ...
      if (sources.length > 0 && !sources.every(s => s === "'self'" || s === "'none'")) {
        return true;
      }
    }
  }
 ...
 * @returns true if embedding is blocked, false otherwise.
 */
function isBlockedByCOEP(value) {
  if (!value) {
    return false;
  }
 ...
 */
async function checkEmbeddability(url) {
  const match = (0, _embeds.getMatchingEmbed)(_embeds2.default, url);
  if (!match) {
    return {
      embeddable: false,
      reason: "no-match"
  ...
  try {
    ...
    const timeoutId = setTimeout(() => controller.abort(), _time.Second.ms * 3);
 ...
 */
function isEmbedUrl(url, embedDescriptors) {
 ...
 */
function convertBareUrlsToEmbedMarkdown(text) {
  let embedDescriptors = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : _embeds2.default;
