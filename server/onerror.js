...
function onerror(app) {
  ...
  app.context.onerror = function (err) {
    ...
    } else if (err.code === "HPE_INVALID_EOF_STATE" || err.code === "ECONNRESET" || err.code === "EPIPE") {
      err = (0, _errors.ClientClosedRequestError)();
    }
    ...
    const shouldReport = err.isReportable === true || err.isReportable !== false && (typeof err.status !== "number" || !_nodeHttp.default.STATUS_CODES[err.status] || err.status === 500);
    if (shouldReport) {
    ...
    if (this.type === "text/html") {
      this.body = readErrorFile().toString().replace(/\/\/inject-message\/\//g, (0, _compat.escape)(err.message)).replace(/\/\/inject-status\/\//g, (0, _compat.escape)(err.status)).replace(/\/\/inject-stack\/\//g, (0, _compat.escape)(err.stack));
    } else {
      this.body = JSON.stringify({
        ok: false,
        error: (0, _compat.snakeCase)(err.id),
...
function readErrorFile() {
  if (_env.default.isDevelopment) {
    return _nodeFs.default.readFileSync(_nodePath.default.join(__dirname, "error.dev.html"));
  }
  if (_env.default.isProduction) {
    return errorHtmlCache ?? (errorHtmlCache = _nodeFs.default.readFileSync(_nodePath.default.join(__dirname, "error.prod.html")));
  }
  return errorHtmlCache ?? (errorHtmlCache = _nodeFs.default.readFileSync(_nodePath.default.join(__dirname, "static/error.dev.html")));
}