...
var _ChangesetHelper = require("../../../shared/editor/lib/ChangesetHelper");
...
let DocumentHelper = exports.DocumentHelper = (_dec = (0, _tracing.trace)(), _dec(_class = class DocumentHelper {
  /**
  ...
  static async toJSON(document, options) {
    ...
    if ("content" in document && document.content) {
      ...
      doc = _editor.parser.parse("text" in document ? document.text ?? "" : "");
    }
  ...
  static toPlainText(document) {
    ...
    return (0, _textBetween.default)(node, 0, node.content.size);
  }
   ...
   */
  static async toMarkdown(document, options) {
    let node = DocumentHelper.toProsemirror(document);
    if (options?.signedUrls && options?.teamId) {
      const data = await _ProsemirrorHelper.ProsemirrorHelper.signAttachmentUrls(node, options.teamId, options.signedUrls);
      node = _prosemirrorModel.Node.fromJSON(_editor.schema, data);
    }
    const text = _editor.serializer.serialize(node).replace(/(^|\n)\\(\n|$)/g, "\n\n").trim();
    if ((document instanceof _.Collection || document instanceof _.Document || document instanceof _.Revision) && options?.includeTitle !== false) {
      const iconType = (0, _icon.determineIconType)(document.icon);
      const name = document instanceof _.Collection ? document.name : document.title;
      const title = `${iconType === _types.IconType.Emoji ? document.icon + " " : ""}${name}`;
      return `# ${title}\n\n${text}`;
    }
    return text;
  }
   ...
   */
  static async toHTML(model, options) {
    const node = DocumentHelper.toProsemirror(model);
    let output = await _ProsemirrorHelper.ProsemirrorHelper.toHTML(node, {
      title: options?.includeTitle !== false ? model instanceof _.Collection ? model.name : model.title : undefined,
    ...
    (0, _tracer.addTags)({
      collectionId: model instanceof _.Collection ? model.id : undefined,
      documentId: !(model instanceof _.Collection) ? model.id : undefined
    });
    if (options?.signedUrls) {
      const teamId = model instanceof _.Collection || model instanceof _.Document ? model.teamId : (await model.$get("document"))?.teamId;
      if (!teamId) {
        return output;
      }
  ...
  static async diff(before, after) {
    ...
    const changeset = _ChangesetHelper.ChangesetHelper.getChangeset(afterJSON, beforeJSON);
  ...
  static async toEmailDiff(before, after, options) {
    ...
    const containsDiffElement = node => {
      if (!node) {
        return false;
      }
      const diffClasses = [_EditorStyleHelper.EditorStyleHelper.diffInsertion, _EditorStyleHelper.EditorStyleHelper.diffDeletion, _EditorStyleHelper.EditorStyleHelper.diffModification, _EditorStyleHelper.EditorStyleHelper.diffNodeInsertion, _EditorStyleHelper.EditorStyleHelper.diffNodeDeletion, _EditorStyleHelper.EditorStyleHelper.diffNodeModification];
      return diffClasses.some(className => node.classList.contains(className) || node.querySelector(`.${className}`));
    };
  ...
  static applyMarkdownToDocument(document, text) {
    let editMode = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : _types.TextEditMode.Replace;
    let findText = arguments.length > 3 ? arguments[3] : undefined;
    let doc;
    if (editMode === _types.TextEditMode.Patch) {
      if (!findText) {
        throw (0, _errors.ValidationError)("findText is required when using patch edit mode");
      }
      const existingDoc = DocumentHelper.toProsemirror(document);
      const {
        markdown,
        blockMap
      } = _editor.serializer.serializeWithPositions(existingDoc);
      const matchIndex = markdown.indexOf(findText);
      if (matchIndex === -1) {
        throw (0, _errors.ValidationError)("The specified text was not found in the document");
      }
      const matchEnd = matchIndex + findText.length;

      // Find which top-level blocks overlap the matched range
      const affected = blockMap.filter(b => b.mdTo > matchIndex && b.mdFrom < matchEnd);
      if (affected.length === 0) {
        throw (0, _errors.ValidationError)("Could not map the matched text to document content");
      }
      ...
      const surgicalResult = affected.length === 1 ? DocumentHelper.trySurgicalPatch(existingDoc, pmFrom, pmTo, patch) : undefined;
      if (surgicalResult) {
        doc = surgicalResult;
      } else {
        ...
        const modifiedRegion = regionMarkdown.slice(0, localMatchStart) + text + regionMarkdown.slice(localMatchEnd);
        const newContent = _editor.parser.parse(modifiedRegion);
    ...
    } else if (editMode === _types.TextEditMode.Append) {
      const existingDoc = DocumentHelper.toProsemirror(document);
      const newDoc = _editor.parser.parse(text);
      const lastChild = existingDoc.lastChild;
      const firstChild = newDoc.firstChild;
      if (!text.match(/^\s*\n/) && lastChild && firstChild && lastChild.type.name === "paragraph" && firstChild.type.name === "paragraph") {
        const mergedPara = lastChild.copy(lastChild.content.append(firstChild.content));
        doc = existingDoc.copy(existingDoc.content.cut(0, existingDoc.content.size - lastChild.nodeSize).append(_prosemirrorModel.Fragment.from(mergedPara)).append(newDoc.content.cut(firstChild.nodeSize)));
      } else {
        doc = existingDoc.copy(existingDoc.content.append(newDoc.content));
      }
    } else if (editMode === _types.TextEditMode.Prepend) {
      const existingDoc = DocumentHelper.toProsemirror(document);
      const newDoc = _editor.parser.parse(text);
      const lastChild = newDoc.lastChild;
      const firstChild = existingDoc.firstChild;
      if (!text.match(/\n\s*$/) && lastChild && firstChild && lastChild.type.name === "paragraph" && firstChild.type.name === "paragraph") {
        const mergedPara = lastChild.copy(lastChild.content.append(firstChild.content));
        doc = existingDoc.copy(newDoc.content.cut(0, newDoc.content.size - lastChild.nodeSize).append(_prosemirrorModel.Fragment.from(mergedPara)).append(existingDoc.content.cut(firstChild.nodeSize)));
      } else {
        doc = existingDoc.copy(newDoc.content.append(existingDoc.content));
      }
    } else {
      doc = _editor.parser.parse(text);
    }
    document.content = doc.toJSON();
    document.text = _editor.serializer.serialize(doc);
    if (document.state) {
      const ydoc = new Y.Doc();
      Y.applyUpdate(ydoc, document.state);
   ...
   */
  static trySurgicalPatch(existingDoc, pmFrom, pmTo, patch) {
    ...
    const before = existingDoc.content.cut(0, pmFrom);
    const after = existingDoc.content.cut(pmTo);
    return existingDoc.copy(before.append(_prosemirrorModel.Fragment.from(patchedBlock)).append(after));
  }
  ...
  static patchNode(node, patch) {
    ...
    const modifiedMd = containerMd.slice(0, localStart) + replacementText + containerMd.slice(localEnd);
    const parsed = _editor.parser.parse(modifiedMd.replace(/^\n+/, ""));
    ...
    const originalParsed = _editor.parser.parse(containerMd.replace(/^\n+/, ""));
    const roundTripped = DocumentHelper.findChildOfType(originalParsed, node.type);
   ...
   */
  static findChildOfType(doc, type) {
    let result;
    doc.forEach(child => {
   ...
   */
  static mergeNodes(original, updated, roundTripped) {
    ...
    const newChildren = [];
    const rtChildren = [];
    original.forEach(child => oldChildren.push(child));
    updated.forEach(child => newChildren.push(child));
    roundTripped?.forEach(child => rtChildren.push(child));
    ...
    for (let i = 0; i < oldChildren.length; i++) {
      ...
      if (textSame && oldChild.sameMarkup(newChild)) {
        ...
        const mergedAttrs = DocumentHelper.mergeAttrs(oldChild, newChild, rtChild);
        merged.push(oldChild.type.create(mergedAttrs, oldChild.content, oldChild.marks));
      } else if (!oldChild.isTextblock && !oldChild.isLeaf) {
    ...
    const mergedAttrs = DocumentHelper.mergeAttrs(original, updated, roundTripped);
    return original.type.create(mergedAttrs, _prosemirrorModel.Fragment.from(merged), original.marks);
  }
   ...
   */
  static mergeAttrs(original, updated, roundTripped) {
    if (!roundTripped) {
   ...
   */
  static tryInlinePatch(blockNode, patch) {
    const {
      markdown,
      matchIndex,
      matchEnd,
    ...
    const separatorLen = blockMdRaw.length - blockMdRaw.replace(/^\n+/, "").length;
    ...
    blockNode.forEach((child, offset) => {
      const childTextLen = child.isText ? child.text.length : child.type.spec.leafText ? child.type.spec.leafText(child).length : 0;
      segments.push({
        textFrom: textOffset,
    ...
    const contentMd = markdown.slice(contentMdStart, contentMdStart + blockText.length);
    ...
    const hasOverlappingAtom = segments.some(seg => seg.isAtom && seg.textTo > localTextFrom && seg.textFrom < localTextTo);
    ...
    const parsed = _editor.parser.parse(replacementText);
    const firstBlock = parsed.firstChild;
    if (parsed.childCount !== 1 || !firstBlock?.isTextblock) {
      return undefined;
    }
    let replacementContent;
    if (firstBlock.content.childCount === 1 && firstBlock.firstChild?.isText && !firstBlock.firstChild.marks.length) {
      // Plain text replacement — use the raw string to avoid whitespace trimming
      replacementContent = _prosemirrorModel.Fragment.from(_editor.schema.text(replacementText));
    } else {
    ...
    return blockNode.copy(inlineBefore.append(replacementContent).append(inlineAfter));
  }
   ...
   */
  static textToPmOffset(segments, textPos) {
   ...
   *
   **/
  static sortDocumentsByStructure(documents, documentStructure) {
