...
exports.default = presentNavigationNode;
/**
 ...
 */
function presentNavigationNode(team, node) {
  return {
    id: node.id,
    title: node.title,
    url: /^https?:\/\//.test(node.url) ? node.url : new URL(node.url, team.url).href,
    children: (node.children ?? []).map(child => presentNavigationNode(team, child))
  };
}