...
router.post("userMemberships.list", (0, _authentication.default)(), (0, _pagination.default)(), (0, _validate.default)(T.UserMembershipsListSchema), async ctx => {
  ...
  const memberships = await _models.UserMembership.scope("withUser").findAll({
    where: {
      userId: user.id,
      documentId: {
  ...
  const documents = await _models.Document.findByIds(documentIds, {
    userId: user.id
  });
...
router.post("userMemberships.update", (0, _authentication.default)(), (0, _validate.default)(T.UserMembershipsUpdateSchema), (0, _transaction.transaction)(), async ctx => {
  ...
  const membership = await _models.UserMembership.scope("withUser").findByPk(id, {
    transaction,
    lock: {
      level: transaction.LOCK.UPDATE,
      of: _models.UserMembership
    },
    rejectOnEmpty: true
  });
