...
const renderEmbed = async (ctx, next) => {
  ...
  if (!url) {
    ctx.throw((0, _errors.InvalidRequestError)("url is required"));
  }
  let parsed;
  try {
    parsed = new URL(url);
  } catch (_err) {
    ctx.throw((0, _errors.InvalidRequestError)("Invalid URL provided"));
  }
...
`;
  ...
  if ((parsed.host === "pinterest.com" || parsed.host.endsWith(".pinterest.com")) && parsed.protocol === "https:" && ctx.path === "/embeds/pinterest") {
