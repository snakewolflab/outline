...
exports.getMarkRangeNodeSelection = getMarkRangeNodeSelection;
/**
 ...
 * @returns An object containing the from and to positions and the mark, or false.
 */
function getMarkRangeNodeSelection(selection, type) {
  const mark = (selection.node.attrs.marks ?? []).find(mark => mark.type === type.name);
  if (!mark) {
    return false;
  }
  return {
