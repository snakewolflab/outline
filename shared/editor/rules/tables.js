 ...
 */
function parseFencedCell(md, content, env) {
  ...
  const unescaped = isRawFence ? (0, _tableCell.unescapeRawTableCell)(source) : source;
  const tokens = md.parse(unescaped, env);
  const type = tokens[0]?.type;
  if (type === "container_notice_open" || type === "container_toggle_open" || type === "fence" || type === "math_block") {
    return tokens;
  }
  return null;
}
function markdownTables(md) {
  ...
  md.core.ruler.after("inline", "tables-pm", state => {
    ...
    for (let i = tokens.length - 1; i > 0; i--) {
      ...
      if (tokens[i].type === "inline" && (tokens[i].content.match(BREAK_REGEX) || tokens[i].content.match(BR_TAG_REGEX))) {
