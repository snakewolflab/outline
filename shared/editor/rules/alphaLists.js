...
exports.default = markdownItAlphaLists;
/**
 ...
 */
function markdownItAlphaLists(md) {
  ...
  md.core.ruler.before("normalize", "alpha_lists_preprocess", state => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (match) {
        ...
        const num = isLowercase ? letter.charCodeAt(0) - 96 // a=1, b=2
    ...
    for (let i = 0; i < state.tokens.length; i++) {
      ...
      if (token.type === "ordered_list_open") {
        ...
        for (let j = i + 1; j < state.tokens.length; j++) {
          ...
          if (itemToken.type === "ordered_list_open" || itemToken.type === "bullet_list_open") {
            break;
          }
        }
      }
