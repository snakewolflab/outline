...
exports.default = exports.ImageSource = void 0;
var _pako = require("pako");
var _pngChunksExtract = _interopRequireDefault(require("png-chunks-extract"));
function _interopRequireDefault(e) { return e && e.__esModule ? e : { default: e }; }
let ImageSource = exports.ImageSource = /*#__PURE__*/function (ImageSource) {
  ImageSource["DiagramsNet"] = "diagrams.net";
  return ImageSource;
}({});
class FileHelper {
  /**
   ...
   */
  static parseSvgDimensions(svgText) {
    ...
    const viewBoxMatch = svgText.match(/\bviewBox=["'][\d.-]+\s+[\d.-]+\s+([\d.]+)\s+([\d.]+)["']/);
    if (viewBoxMatch) {
   ...
   */
  static async getImageSourceAttr(file) {
   ...
   */
  static async isDiagramsNetImage(file) {
    if (file.type === "image/svg+xml") {
   ...
   * @returns True if the file contains Draw.io/Diagrams.net data
   */
  static async isDiagramsNetSvg(file) {
    try {
      const text = await file.text();
      // Check for mxfile content attribute (URL-encoded) or embedded mxfile element
      return text.includes('content="%3Cmxfile') || text.includes("content='%3Cmxfile") || text.includes("<mxfile");
    } catch (_err) {
      return false;
    }
  }
   ...
   * @returns True if the file contains Draw.io/Diagrams.net data
   */
  static async isDiagramsNetPng(file) {
    const buffer = await file.arrayBuffer();
          ...
          try {
            ...
            if (type === "tEXt") {
              ...
              if (compressionMethod === 0) {
                ...
                textContent = (0, _pako.inflate)(compressed, {
                  to: "string"
                });
              } else {
   ...
   */
  static base64ToFile(base64Data, filename, mimeType) {
