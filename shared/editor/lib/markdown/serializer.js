...
class MarkdownSerializer {
  ...
  serializeWithPositions(content, options) {
    const state = new MarkdownSerializerState(this.nodes, this.marks, options);
    state.blockMap = [];
    state.renderContent(content);
    return {
...
class MarkdownSerializerState {
  ...
  set out(value) {
    if (!warnedDirectOutAssignment) {
      ...
      console.warn("MarkdownSerializerState: assigning `out` directly is slow on large documents, use append() instead.");
    }
    this._out = value;
  ...
  constructor(nodes, marks, options) {
    ...
    _defineProperty(this, "lastChar", "");
    _defineProperty(this, "options", void 0);
    _defineProperty(this, "blockMap", null);
    this.nodes = nodes;
    this.marks = marks;
  ...
  text(text, escape) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      this.write();
      this.append(escape !== false ? this.esc(lines[i], startOfLine) : lines[i]);
      if (i !== lines.length - 1) {
        this.append("\n");
      }
    }
  ...
  renderTable(node) {
    ...
    node.forEach((row, _, i) => {
      row.forEach((cell, _, j) => {
        ...
        const cellState = new MarkdownSerializerState(this.nodes, this.marks, this.options);
        ...
        const content = cellState.out.replace(/^\n+|\n+$/g, "").replace(/\n/g, "<br>");
        this.append(content);
